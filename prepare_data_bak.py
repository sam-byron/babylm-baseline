...
def prepare_data(config, tokenizer, cache_path):
    ...
    tokenize_with_tokenizer = tokenizer

    # glob for all .md files in bnc folder
    data_files = {"train": glob.glob("./data/pretrain/bnc/**/*.md", recursive=True)}
    print(f"Found {len(data_files['train'])} .md files")
    # 2) Load as text, sampling by paragraph
    ds = load_dataset(
        "text",
        data_files=data_files,
        # sample_by="paragraph",
        # keep_linebreaks=True,  # each line is already one sentence
        streaming=False,
        split="train",
