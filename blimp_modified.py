...
def parse_arguments():
    ...
    parser.add_argument("--checkpoint_path", default="/home/sam-byron/engineering/ML/playground/babylm/bnc/bl-bnc-custom-ltg-bert/model_vault/bert_ltg_orig_bl_128_run_60_60_cls_sep", type=str, help="The initial checkpoint to start training from.")
...
def is_right(good, bad, model, tokenizer, device):
    ...
    good_ids = [tokenizer.token_to_id(token) for token in good_tokens if tokenizer.token_to_id(token) is not None]
    bad_ids = [tokenizer.token_to_id(token) for token in bad_tokens if tokenizer.token_to_id(token) is not None]
    # Convert to tensors
...
def evaluate_all(model, tokenizer, blimp, device):
    ...
    for group_key, group in blimp.items():
        for subgroup in group:
            total_correct += evaluate(model, tokenizer, subgroup, device)
            total += 1
    ...
    # wandb.run.summary["BLiMP/accuracy"] = total_correct / total
    print(f"BLiMP accuracy: {(total_correct / total)*100} %", flush=True)
