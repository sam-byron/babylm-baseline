...
from mlm_dataset import BasicDataset, DocumentDataset, OrderDataset
...
def parse_arguments():
    ...
    parser.add_argument("--output_dir", default="checkpoints/standart_subword", type=str, help="The output directory where the model checkpoints will be written.")
    ...
    parser.add_argument("--run_name", default="standart_subword", type=str, help="Name of the run as logged into wandb.")

    # Other parameters
    parser.add_argument("--optimizer", default="lamb", type=str)
    parser.add_argument("--masking", default="subword", type=str)
    parser.add_argument("--nsp", default="basic", type=str)
    ...
    parser.add_argument("--max_steps", default=2000000, type=int, help="Total number of training steps to perform.")
...
def setup_training(args):
    ...
    args.n_training_files = len(fnmatch.filter(os.listdir(args.input_dir), "train_*.pickle.gz"))
    if is_main_process():
...
def prepare_model_and_optimizer(args, device, local_rank, checkpoint):
    ...
    model = Bert(config, args.nsp, args.activation_checkpointing)
    ...
    optimizer_grouped_parameters = [
        ...
        {'params': [p for _, p in high_decay_params], 'weight_decay': args.head_weight_decay},
    ...
    if is_main_process():
        ...
        for n, _ in no_decay_params:
            print(n)
        print()
        print("Parameters with high weight decay:")
        for n, _ in high_decay_params:
            print(n)
        print(flush=True)
...
def training_epoch(model, data, optimizer, scheduler, grad_scaler, global_step, epoch, args, device, max_local_steps):
    ...
    for local_step, batch in enumerate(train_iter):
        input_ids, attention_mask, target_ids, is_next_sentence = [t.to(device, non_blocking=True) for t in batch]
        input_ids, target_ids = input_ids.t(), target_ids.t()

        with torch.cuda.amp.autocast(args.mixed_precision):
            prediction, next_sentence_p = model(input_ids, attention_mask, target_ids)

            target_ids = target_ids.flatten()
            target_ids = target_ids[target_ids != -100]
            subword_loss = F.cross_entropy(prediction, target_ids)

            if next_sentence_p is not None:
                next_sentence_loss = F.binary_cross_entropy_with_logits(next_sentence_p, is_next_sentence.float())
                loss = subword_loss + next_sentence_loss
            else:
                loss = subword_loss
        ...
        with torch.no_grad():
            ...
            if next_sentence_p is not None:
                next_sentence_accuracy = ((next_sentence_p > 0.0) == is_next_sentence).float().mean()
                ...
                wandb.log(
                    {
                        ...
                        "train/next_sentence_loss": next_sentence_loss.item() if next_sentence_p is not None else 0.0,
                        "train/accuracy": accuracy.item() * 100.0,
                        "train/next_sentence_accuracy": next_sentence_accuracy.item() * 100.0 if next_sentence_p is not None else 0.0,
    ...
    def schedule_next_job(path):
        def signal_handler(sig, frame):
            if get_rank() == 0:
                ...
                wandb.run.finish()
            if get_rank() == 1:
                os.system(f"sbatch --dependency=afterany:{os.getenv('SLURM_JOBID')} train.sh --checkpoint_path {path}")
                exit()
...
def load_dataset(args, epoch, global_step, device):
    ...
    train_path = f"{args.input_dir}/train_{train_index}.pickle.gz"
    seq_len = args.seq_length * 4 if global_step >= int(args.device_max_steps * args.long_after) else args.seq_length

    dataset_class = {"basic": BasicDataset, "document": DocumentDataset, "order": OrderDataset}[args.nsp]
    train_data = dataset_class(train_path, tokenizer, args.masking, seq_length=seq_len, mask_p=args.mask_p, short_p=args.short_p)
...
if __name__ == "__main__":
    ...
    if args.checkpoint_path is not None:
        ...
        args.wandb_id = wandb.util.generate_id() if is_main_process() else 0
    ...
    if get_rank() == 1:
        os.system(f"sbatch --dependency=afterany:{os.getenv('SLURM_JOBID')} --output=eval_{args.run_name} --job-name=eval_{args.run_name} evaluate.sh {checkpoint_path}")
